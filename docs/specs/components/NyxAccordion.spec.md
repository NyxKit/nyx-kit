...
**Status:** Draft proposal, not implemented or exported. Created 2026-09-12 on `017-nyx-accordion`.

Feature scenarios: [017-nyx-accordion](../../../specs/017-nyx-accordion/spec.md). This living document is the authoritative proposed API; future stories must match it.
...
The proposed mode prop is `multiple`, defaulting to `false`. Opening an item closes its siblings by default. Setting `multiple` allows independent expansion. A parent v-model is optional in either mode.

An explicit mode keeps behavior stable when the model is absent, initially undefined, or cleared. Inferring mode from the model alone would make unbound multiple mode impossible to express. A second inverse `autoClose` prop would create conflicting settings, so the proposal uses only `multiple`.
...
- NyxTabs keeps all panels mounted and defaults to the first tab. Accordion will also preserve mounted content, but begins fully collapsed and places each body directly after its header.
...
Planned structure: `src/components/NyxAccordion/` containing `NyxAccordion.vue`, `NyxAccordion.types.ts`, `NyxAccordion.scss`, `NyxAccordion.stories.ts`, and `NyxAccordion.spec.ts`.
...
- Keep bodies mounted while items exist. Collapsed bodies use native hidden behavior so descendants are absent from the tab order and accessibility tree. Closing and reopening retains child component/form state.
...
During implementation planning, evaluate a small private panel component responsible for the body wrapper, caller-supplied IDs/ARIA attributes, and forwarding body content. Slot selection belongs to each public parent; a shared panel should receive its resolved content through a default slot. Extract only if it meaningfully removes duplication after both usages are sketched.
...
Only theme and size are proposed visual controls for the first version. No new global tokens or dependencies are required by this spec.

The shared docs describe `Default` enum sentinels that are absent from current `src/types/common.ts`; this discrepancy is recorded in [AGENTS.md](../../../AGENTS.md). This proposal uses optional props and does not require adding enum members. Reconcile shared-default documentation before implementing against that guidance.
...
When implemented, export the component through direct `.vue` imports in `src/components/index.ts` and `src/index.ts`. Export public item, model, slot, and props types through `src/types/index.ts` and the existing root type exports, following the package contract. Do not add exports before implementation exists.
...
No duplicate `change`, `open`, or `close` events are proposed. This component updates an ID model, not nested item data, so the nested-model semantic-event requirement does not apply.

## Slots

| Slot | Scope | Purpose / fallback |
|---|---|---|
| `item-header-${id}` | `NyxAccordionSlotProps` | Per-item trigger content; falls back to `header`, then `item.label` |
...
A missing body renders an empty body without developer instructions in the product UI. Keep the body element for a valid `aria-controls` target. An empty item list without an `empty` slot creates no fake trigger or panel. No additional root header/footer API is proposed.

## Usage examples

Proposed API examples; these do not import an implemented component yet.
...
- Browser tests: real focus movement, native Space/Enter activation without double toggles or form submission, hidden-panel tab exclusion, nested accordions, and focus recovery on external collapse/removal.
...
This is a draft contract; implementation and stories are a subsequent step. Proposed defaults beyond the requested single/multiple behavior include all-collapsed initialization, descriptor-based items, retained mounted bodies, and the slot names above.

The first version is vertical only. Lazy mounting, unmount-on-close, mandatory-open mode, expand/collapse-all controls, per-item action buttons, public item/panel subcomponents, animation configuration, and model-type inference are outside this proposal. Sharing a private panel is an evaluation item, not a prerequisite for shipping the accordion.
